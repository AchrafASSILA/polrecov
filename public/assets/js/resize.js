...
(function (factory) {
        ...
        for (var prop in fallBackSetting) {
            ...
            if (userSetting.hasOwnProperty(prop)) {
                ...
                if (typeof userObject === "object") {
                    resultObject[prop] = settingsFallback(
                        userObject,
                        fallBackSetting[prop]
                    );
                } else {
    ...
    $.extend(ColResize.prototype, {
        fnEnable: function () {
            if (this.isEnabled) {
                this.s.dt.oInstance.oApi._fnLog(
                    this.dt,
                    1,
                    "ColResize attempted to enable again. Ignoring."
                );
                return;
            }
            this._fnConstruct();
        ...
        fnReset: function () {
            ...
            this.s.opts.onResizeEnd(
                null,
                this._fnGetAllColumns().map(this._fnMapColumn)
            );
        },
        fnDisable: function () {
            if (!this.isEnabled) {
                this.s.dt.oInstance.oApi._fnLog(
                    this.dt,
                    1,
                    "ColResize attempted to disable again. Ignoring."
                );
                return;
            }
        ...
        _fnConstruct: function () {
            ...
            $(document).on(
                "mousemove.ColResize touchmove.ColResize",
                function (e) {
                    if (that.s.state.isDragging) {
                        var changedWidth =
                            that._fnGetXCoords(e) - that.s.state.startX;
                        that._fnApplyWidth(changedWidth);

                        that.s.opts.onResize(
                            that._fnMapColumn(that.s.state.column)
                        );
            ...
            $(document).on("mouseup.ColResize touchend.ColResize", function () {
                if (that.s.state.isDragging) {
                    ...
                    // callback
                    var mappedColumns = that
                        ._fnGetAllColumns()
                        .map(that._fnMapColumn);
                    that.s.opts.onResizeEnd(
                        that._fnMapColumn(that.s.state.column),
                        mappedColumns
                    );
                }
            ...
            this._fnGetAllColumns().forEach(function (column) {
                ...
                if (isResizable) {
                    ...
                    $columnNode.on(
                        "mousedown.ColResize touchstart.ColResize",
                        function (e) {
                            var $node = $(e.currentTarget);
                            if (that._fnIsInDragArea($node, e)) {
                                ...
                                that.s.state.minWidth = that._fnGetWidthOfValue(
                                    $node.css("min-width")
                                );
                                that.s.state.maxWidth = that._fnGetWidthOfValue(
                                    $node.css("max-width")
                                );
                                ...
                                that.s.state.isLastColumnDragging =
                                    that._fnIsLastResizableColumnDragging(
                                        column
                                    );

                                //disable sorting
                                that._fnGetAllColumns().forEach(function (
                                    column
                                ) {
                                    column._bSortableTempHolder =
                                        column.bSortable;
                                    column.bSortable = false;
                                });
                                that.s.opts.onResizeStart(
                                    null,
                                    that
                                        ._fnGetAllColumns()
                                        .map(that._fnMapColumn)
        ...
        _fnApplyWidth: function (changedWidth) {
            ...
            changedWidth = this.s.opts.hasBoundCheck
                ? this._fnBoundCheck(changedWidth)
                : changedWidth;
            ...
            var shouldChangeTableWidth =
                changedWidth < 0 &&
                this.s.state.$element.closest(".dataTables_scroll").length >
                    0 &&
                $table.width() + changedWidth >
                    this.s.state.$element.closest(".dataTables_scroll").width();
            if (shouldChangeTableWidth) {
                $table.width($table.width() + changedWidth - 1);
            }

            var scrollBodyTh = this.s.state.$element
                .closest(".dataTables_scroll")
                .find(
                    ".dataTables_scrollBody table th:nth-child(" +
                        (this.s.state.$element.index() + 1) +
                        ")"
                );
        ...
        _fnBoundCheck: function (changedWidth) {
            ...
            //min bound
            if (
                this.s.state.minWidth !== -1 &&
                thWishWidth < this.s.state.minWidth
            ) {
                ...
                if (this.s.state.minBoundAllowClass) {
                    ...
                    setTimeout(function () {
                        $currentElement.removeClass(that.s.opts.minBoundClass);
                    }, 500);
            ...
            //max bound
            if (
                this.s.state.maxWidth !== -1 &&
                thWishWidth > this.s.state.maxWidth
            ) {
                ...
                if (this.s.state.maxBoundAllowClass) {
                    ...
                    $currentElement = this.s.state.$element;
                    setTimeout(function () {
                        $currentElement.removeClass(that.s.opts.maxBoundClass);
                    }, 500);
        ...
        _fnIsLastResizableColumnDragging: function (draggingColumn) {
            var visibleColumns = this._fnGetAllColumns().filter(function (
                column
            ) {
            ...
            for (
                var counter = indexOfColumn + 1;
                counter < visibleColumns.length;
                counter++
            ) {
            ...
            fnInit: function (settings) {
                ...
                if (!settings._colResize) {
                    ...
                    table.oApi._fnLog(
                        settings,
                        1,
                        "ColResize attempted to initialise twice. Ignoring second"
                    );
                }
